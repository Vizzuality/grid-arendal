...
When /^(?:|I )filter active on (.+)$/ do |page_name|
  visit path_to(page_name) + '?active=true'
end

When /^(?:|I )filter inactive on (.+)$/ do |page_name|
...
Then(/^the "([^"]*)" field(?: within "([^"]*)")? should contain "([^"]*)"$/) do |field, selector, value|
  with_scope(selector) do
    ...
    if field_value.respond_to? :should
      expect(field_value).to be =~ /#{value}/
    else
...
Then /^I wait for the ajax request to finish$/ do
  start_time = Time.now
  expect(page.evaluate_script('jQuery.isReady&&jQuery.active==0').class).to_not eql(String) until page.evaluate_script('jQuery.isReady&&jQuery.active==0') or (start_time + 5.seconds) < Time.now do
    sleep 1
  end
end
